...
def calibrate(I0, A, R, qm, rl,  Bs, B_dict, T, scalar, IF, Imax, success_emp, num_years, min_value, tolerance=.9, parallel_processes=2):
    ...
    while np.sum(GoF_alpha<tolerance) > 0 or np.sum(GoF_beta<tolerance) > 0:
        ...
        errors = np.array(compute_error(I0=I0, alphas=alphas, betas=betas, A=A, R=R, qm=qm, rl=rl,  
                                Bs=Bs, B_dict=B_dict, T=T, scalar=scalar, IF=IF, Imax=Imax, success_rates=success_emp, 
