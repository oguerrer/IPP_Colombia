...
Corrige las extrapolaciones que superen los límites teóricos.
...
df = pd.read_csv(home+"data/preprocessed/indicators_imputed.csv")
...
dfn = pd.read_csv(home+"data/preprocessed/indicators_normalized.csv")
...
for index in df.index.values:
    ...
    if np.sum(row_imputed[colYears].isnull()) == 0 and len(missing_values) > 0:
        ...
        if len(vvf) > 0:
            ...
            first_more_than_ub = np.sum(vals[0:first_observation]>.9) > 0
            first_less_than_lb = np.sum(vals[0:first_observation]<.1) > 0
        ...
        if len(vvl) > 0:
            ...
            last_more_than_ub = np.sum(vals[last_observation+1::]>.9) > 0
            last_less_than_lb = np.sum(vals[last_observation+1::]<.1) > 0
...
dff.to_csv(home+"data/preprocessed/indicators_corrected.csv", index=False)
