...
Corre simulaciones contrafactuales en las que el presupuesto total aumenta en
distintos niveles.
...
for interest in range(5, 21, 5):
        
    Bs = [[] for g in range(len(df_expt))]
    for i, year in enumerate(range(2021, 2020+years_forward+1)):
        for j in range(sub_periods):
            for index, row in df_expt.iterrows():
                Bs[index].append( (1+interest/100)*Bs_retro_tot[index,i]/sub_periods )
    Bs = np.array(Bs)

    
    print('Runing model ...', interest, np.sum(Bs))
    ...
    dfi.to_csv(home+"data/sims/increment_"+str(interest)+".csv", index=False)
