...
Corre simulaciones en la frontera presupuestaria.
...
sols = np.array(Parallel(n_jobs=parallel_processes, verbose=0)(delayed(run_ppi_parallel)\
            (I0, alphas, betas, A, R, qm, rl, Imax, Bs, B_dict, T, scalar, frontier=np.ones(N)) for itera in range(sample_size)))
...
dfi.to_csv(home+"data/sims/frontier.csv", index=False)
